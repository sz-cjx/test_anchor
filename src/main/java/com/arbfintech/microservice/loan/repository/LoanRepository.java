...
public interface LoanRepository extends JpaRepository<Loan, String>, JpaSpecificationExecutor<Loan> {
    ...
    @Query(value = "SELECT * FROM loan WHERE category=?1 AND locked_operator_no IS NULL AND loan_status IN (?2) ORDER BY create_time DESC;", nativeQuery = true)
    List<Loan> findAllByCategoryOrderByCreateTimeDesc(Integer category,List<Integer> loanStatus);

    @Query(value = "SELECT * FROM loan WHERE  category=?1 AND priority<=?2 AND locked_operator_no IS NULL AND loan_status in (?3) ORDER BY create_time DESC,priority DESC;", nativeQuery = true)
    List<Loan> findLoansByCategoryAndPriority(Integer category, Integer piority,List<Integer> loanStatus);
