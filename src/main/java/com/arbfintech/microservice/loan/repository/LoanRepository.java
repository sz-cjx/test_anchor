...
public interface LoanRepository extends JpaRepository<Loan, String>, JpaSpecificationExecutor<Loan> {
    ...
    @Query(value = "SELECT * FROM loan WHERE category=?1 AND locked_operator_no IS NULL AND loan_status IN (?2) AND operator_no!=?3 ORDER BY create_time DESC;", nativeQuery = true)
