...
public interface LoanRepository extends JpaRepository<Loan, String>, JpaSpecificationExecutor<Loan> {
    ...
    @Query(value = "SELECT * FROM loan WHERE category=?1 AND contract_no like 'IC%' AND locked_operator_no IS NULL AND loan_status IN (?2) AND ( operator_no!=?3 or operator_no is NULL ) ORDER BY create_time DESC;", nativeQuery = true)
    List<Loan> findAllByCategoryOrderByCreateTimeDesc(Integer category,List<Integer> loanStatus,String operatorNo);

    @Query(value = "SELECT * FROM loan WHERE  category=?1 AND contract_no like 'IC%' AND priority<=?2 AND locked_operator_no IS NULL AND loan_status in (?3) AND operator_no!=?4 ORDER BY create_time DESC,priority DESC;", nativeQuery = true)
